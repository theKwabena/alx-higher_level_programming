...
- What is OOP
- 'first-css and an object or instance
...
- What is the `__dict__` of a class and/or instance of a class and what does 
...
## Task files descriptions:
  ...
  * [1-rectangle.py](./1-rectangle.py): Python class that defines a rectangle. Builds on
  [0-rectangle.py](./0-rectangle.py) with:
    ...
    * Property getter `def height(self):` to ight`.
  ...
  * Determines all possible solutions for placing N non-attacking queens on an
  NxN chessbtherwise, the program prints
  ...
  * Solutions are printed one per line in the format `[[r, c], [r, c], [r, c],
  [r, c]]` where `r` and `c` represent the row and column, respre a
